...
C1 = cp.Variable(ntimes, pos=True, name="C1")

u1 = cp.Variable(ntimes, pos=True, name="u1")
u2 = cp.Variable(ntimes, pos=True, name="u2")
u3 = cp.Variable(ntimes, pos=True, name="u3")
...
constraints += [C1[t+1] == C1[t] + dt * u3[t] for t in range(ntimes-1)]
constraints += [u1[t] + u2[t] + u3[t] == g[t] for t in range(ntimes)]
objective = cp.Maximize(R1[-1])

iterations = 0
F2 = [0 for _ in range(ntimes)]
C2 = [0 for _ in range(ntimes)]
while True:
  if iterations != 0:
    F2 = F1.value.copy()
    C2 = C1.value.copy()
  mmc = [michaelis_menten_constraint(g[t], F1[t], β1=h, β2=0.1*F2[t], β3=a) for t in range(ntimes)]
  # cc = [michaelis_menten_constraint(g[t], C1[t], β1=1, β2=C2[t], β3=1) for t in range(ntimes)]
  cc = []
  problem = cp.Problem(objective, constraints + mmc + cc)
...
axs[0, 1].plot(times, u3.value, label="Comp Control")
axs[0, 1].legend()
...
axs[1, 1].plot(times, C1.value, label="Comp 1")
axs[1, 1].plot(times, F2, label="Leaves 2")
axs[1, 1].legend()

plt.show()

# There are two plants both in the original and competitive system